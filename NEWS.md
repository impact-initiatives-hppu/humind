...
- A marvelous new function now allows to add (weighted) quantile intervals columns for any numeric variables (#516), including for disagregating by income quintiles.
...
- `add_age_cat()` has more intuitive breaks due to a fix in `num_cat()`. To be able to have the following thresholds: 0-17, 18-59, 60-119, 120+, you can write `add_age_cat("resp_age", breaks = c(0, 18, 60, 120))` (#504).
- If occupation by armed groups was not added in country due to relevancy, `add_loop_edu_disrupted_d()` allows to have a NULL argument there.
