...
fixes inconsistent `NA` handling in `add_hhs()`, corrects the time-to-fetch-water
bucketing in `add_drinking_water_source_cat()`, and removes the runtime
dependency on `impactR.utils`.
