...
dummy_loop_data <- data.frame(
  uuid = c(1, 1, 2, 2, 3, 4),
  edu_disrupted_occupation = c("yes", "no", "dnk", "pnta", "yes", "no"),
...
test_that("add_loop_edu_disrupted_d function works with default parameters", {
  ...
  expect_true(all(c("edu_disrupted_occupation_d", "edu_disrupted_hazards_d", "edu_disrupted_displaced_d", "edu_disrupted_teacher_d") %in% colnames(result)))
  expect_equal(result$edu_disrupted_occupation_d[1], 1)
...
missing_column_data <- dummy_loop_data %>% select(-edu_disrupted_occupation)
...
test_that("add_loop_edu_disrupted_d_to_main function works with default parameters", {
  ...
  expect_true(all(c("edu_disrupted_occupation_n", "edu_disrupted_hazards_n", "edu_disrupted_displaced_n", "edu_disrupted_teacher_n") %in% colnames(main_result)))
  expect_equal(main_result$edu_disrupted_occupation_n[1], 1)
...
test_that("add_loop_edu_disrupted_d_to_main function handles missing columns", {
  expect_error(add_loop_edu_disrupted_d_to_main(missing_column_main_data, dummy_loop_data))
})
...
edge_case_loop_data <- data.frame(
  uuid = c(1, 2),
  edu_disrupted_occupation = c("no", "no"),
...
test_that("add_loop_edu_disrupted_d function handles edge cases", {
  ...
  expect_equal(result$edu_disrupted_occupation_d, c(0, 0))
