...
sum_vars <- function(
  ...
  if (length(new_colname) != 1) {
    rlang::abort("new_colname must be of length 1")
  }
  ...
  if (imputation == "weighted.median") {
    ...
    rlang::abort(
      "The imputation method is not recognized. Please use 'weigthed.median', median', 'value', or 'none'."
    )
  }
