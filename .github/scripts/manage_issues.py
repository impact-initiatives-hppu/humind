...
def are_issues_duplicate(issue1, issue2):
    ...
    desc1 = re.search(r'Description: (.+)', issue1.body)
    desc2 = re.search(r'Description: (.+)', issue2.body)
    ...
    for i, issue in enumerate(all_issues):
        ...
        if issue.state == 'open' and function_name:
            related_issues = [other_issue for other_issue in all_issues 
                              if other_issue != issue and other_issue.state == 'open' and get_function_name(other_issue.body) == function_name]
            if related_issues:
