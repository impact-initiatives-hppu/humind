...
handwashing_facility <- function(df,
  ...
  #------ Check values ranges
  are_values_in_set(df, sanitation_facility, c(handwashing_facility_soap_and_water_codes, handwashing_facility_none_codes, handwashing_facility_na_codes))

  df <- dplyr::mutate(
    df,
    handwashing_facility_class = dplyr::case_when(
      handwashing_facility %in% handwashing_facility_soap_and_water_codes ~ 2,
      handwashing_facility %in% handwashing_facility_none_codes ~ 1,
      .default = NA_real_)
  )
