...
#' @param time_to_fetch_premises Character vector of responses codes, such as "On premises", e.g., c("premises").
#'
...
water_source <- function(df,
                         ...
                         time_to_fetch_premises = c("premises")
                         ) {


  #------ Check values ranges
  are_values_in_set(df, water_source, c(water_source_improved_codes, water_source_unimproved_codes, water_source_surface_water_codes))
  are_values_in_set(df, time_to_fetch, c(time_to_fetch_above_threshold_codes, time_to_fetch_below_threshold_codes, time_to_fetch_premises))
  ...
  df <- dplyr::mutate(
    ...
    time_to_fetch_recoded = dplyr::case_when(
      ...
      {{ time_to_fetch }} %in% time_to_fetch_premises ~ "premises",
      .default = NA_character_)
  )
