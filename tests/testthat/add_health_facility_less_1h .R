...
test_that("values strictly below 60 minutes are coded 1", {
  ...
  expect_equal(result$health_facility_distance_less_1hour[1:3], c(1, 1, 1))
})

test_that("values at or above 60 minutes are coded 0", {
  result <- add_health_facility_less_1h(df)
  expect_equal(result$health_facility_distance_less_1hour[4:6], c(0, 0, 0))
})
...
test_that("fractional values are handled correctly", {
  ...
  expect_equal(result$health_facility_distance_less_1hour, c(1, 0, 0))
})

test_that("NA inputs produce NA output", {
  result <- add_health_facility_less_1h(df)
  expect_true(is.na(result$health_facility_distance_less_1hour[7]))
})
...
test_that("default undefined code -999 is recoded to NA via the < 0 branch", {
  ...
  expect_equal(result$health_facility_distance_less_1hour, c(1, NA_real_))
})
...
test_that("negative values other than -999 are recoded to NA", {
  ...
  expect_equal(
    result$health_facility_distance_less_1hour,
