...
rank_top3_vars <- function(
  ...
  if (length(new_vars_in_df) > 0) {
    rlang::warn(glue::glue(
      "The following variables exist in 'df' and will be replaced: ",
      glue::glue_collapse(new_vars_in_df, sep = ", ", last = " and ")
    ))
  }
  ...
  int <- dplyr::filter(
    int,
    !is.na(!!rlang::sym("val")) & !!rlang::sym("val") > 0
  )
  ...
  int <- dplyr::arrange(
    int,
    !!rlang::sym(id_col),
    dplyr::desc(!!rlang::sym("val")),
    factor(!!rlang::sym("var"), levels = vars)
  )
  ...
  df <- dplyr::select(
    df,
    -dplyr::any_of(c(new_colname_top1, new_colname_top2, new_colname_top3))
  )
