...
#'
#' * *_d: Binary columns for each disruption type (e.g., edu_disrupted_attack_d).
#'
#' @export
add_loop_edu_disrupted_d <- function(
  ...
  if (!is.null(attack)) {
    if_not_in_stop(df, attack, "df")
  }
  if_not_in_stop(df, ind_schooling_age_d, "df")

  # Check if values are in set
  are_values_in_set(df, c(attack, hazards, displaced, teacher), levels)
  ...
  if ("edu_ind_disrupted_cat" %in% colnames(df)) {
    rlang::warn(
      "edu_ind_disrupted_cat already exists in df. It will be replaced."
    )
  }
  ...
  cols <- c(attack, hazards, displaced, teacher)
  df <- dplyr::mutate(
    df,
    dplyr::across(
      dplyr::all_of(cols),
...
#' @param attack_d Column name for the dummy variable of the attack dimension. NULL if dimension is not present in the data frame.
...
add_loop_edu_disrupted_d_to_main <- function(
  ...
  if (!is.null(attack_d)) {
    if_not_in_stop(loop, attack_d, "loop")
  }

  # Check if dummies are 0 and 1
  cols <- c(attack_d, hazards_d, displaced_d, teacher_d)
  are_values_in_set(
    loop,
    cols,
    0:1,
    main_message = "All columns must be binary columns (1s and 0s)."
  )

  # Check if new colnames are in main and throw a warning if it is
  if (!is.null(attack_d) && "edu_disrupted_attack_n" %in% colnames(main)) {
    rlang::warn(
      "edu_disrupted_attack_n already exists in main. It will be replaced."
    )
  }

  if ("edu_disrupted_hazards_n" %in% colnames(main)) {
    rlang::warn(
      "edu_disrupted_hazards_n already exists in main. It will be replaced."
    )
  }

  if ("edu_disrupted_displaced_n" %in% colnames(main)) {
    rlang::warn(
      "edu_disrupted_displaced_n already exists in main. It will be replaced."
    )
  }

  if ("edu_disrupted_teacher_n" %in% colnames(main)) {
    rlang::warn(
      "edu_disrupted_teacher_n already exists in main. It will be replaced."
    )
  }
  ...
  loop_wo_attack <- dplyr::summarize(
    loop,
