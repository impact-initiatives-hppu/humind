...
test_that("Reexport functions are from impactR4PHU", {
  ...
  for (func in expected_functions) {
    expect_true(
      exists(func, envir = humind_ns),
      info = paste("Function", func, "should exist in humind namespace")
    )
    expect_identical(
      environment(get(func, envir = humind_ns)),
      asNamespace("impactR4PHU"),
      info = paste(
        "Function",
        func,
        "should have impactR4PHU namespace as its environment"
      )
    )
  }
})
